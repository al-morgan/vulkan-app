...
gfx::buffer::buffer(const gfx::context& context, size_t size, VkBufferUsageFlags usage) : m_context(context)
{
	...
	vkCreateBuffer(context.device, &create_info, nullptr, &source);
		
	VkMemoryAllocateInfo memory_allocate_info{};
	memory_allocate_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	memory_allocate_info.allocationSize = size;
	memory_allocate_info.memoryTypeIndex = context.memory_type_host_coherent;
	check(vkAllocateMemory(context.device, &memory_allocate_info, nullptr, &source_memory));

	check(vkBindBufferMemory(context.device, source, source_memory, 0));
	
	check(vkMapMemory(context.device, source_memory, 0, size, 0, &memory));

	create_info.usage = usage | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
	vkCreateBuffer(context.device, &create_info, nullptr, &destination);

	memory_allocate_info.memoryTypeIndex = context.memory_type_device_local;
	check(vkAllocateMemory(context.device, &memory_allocate_info, nullptr, &destination_memory));

	check(vkBindBufferMemory(context.device, destination, destination_memory, 0));
}
